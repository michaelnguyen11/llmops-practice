...
hugging_face_deploy_config = {
    "HF_MODEL_ID": settings.HF_MODEL_ID, # Hugging Face model to deploy
    "HUGGING_FACE_HUB_TOKEN": settings.HUGGINGFACE_ACCESS_TOKEN,
    "SM_NUM_GPUS": json.dumps(settings.SM_NUM_GPUS),  # Number of GPU used per replica, which is crucial for fitting your model into the GPU’s VRAM
    ...
    "HF_MODEL_QUANTIZE": "bitsandbytes", # quantization technique to use
}


model_resource_config = ResourceRequirements(
    requests={
        "copies": settings.COPIES,  # Number of replicas. Multiple replicas reduce latency and increase throughput
        "num_accelerators": settings.GPUS,  # Number of GPUs required.
        "num_cpus": settings.CPUS,  # Number of CPU cores required. Handle data pre-processing, post-processing
